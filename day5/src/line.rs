pub mod line {
    ...
    impl Line {
        ...
        pub fn add_intersections(&self, other: &Line, intersection_set: HashSet<(i32,i32)>) -> Result<HashSet<(i32, i32)>, &'static str> {
            if !self.is_overlap(other) {
                return Err("No intersection");
            }
            if !self.is_diagonal() && !other.is_diagonal() {
                // non-diagonal lines
                let mut new_intersection_set: HashSet<(i32, i32)> = intersection_set.clone();
                ...
                if parallel_x {
                    ...
                    for i in start_y..(end_y + 1) {
                        new_intersection_set.insert((fixed_x, i));
                    }
                } else if parallel_y {
                    ...
                    for i in start_x..(end_x + 1) {
                        new_intersection_set.insert((i, fixed_y));
                    }
                } else {
                    ...
                    new_intersection_set.insert((intersect_x, intersect_y));
                }

                return Ok(new_intersection_set);
            }
